...
def add_favorite_people(user_id):
    ...
    # Verifica si el usuario ya agregó este personaje a sus favoritos
    favorite_exists = Favorite.query.filter_by(user_id=user_id, people_id=people_id).first()
    if favorite_exists:
        response_body = {
            "msg": f"El usuario ya agregó a {people.name} a sus favoritos."
        }
